 ...
 * Example usage:
 * ```
 *     return res.activationFail();
 *     // -or-
 *     return res.activationFail(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function activationFail(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.activationFail()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.activationFail()` called with an Error:', optionalData);
